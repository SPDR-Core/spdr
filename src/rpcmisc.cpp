...
// Copyright (c) 2015-2018 The SPDR developers
...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total spdr balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized spdr balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in spdr/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in spdr/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    SpiderDGP spdrDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)spdrDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)spdrDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)spdrDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"spdraddress\"\n"
            "\nReturn information about the given spdr address.\n"
            "\nArguments:\n"
            "1. \"spdraddress\"     (string, required) The spdr address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"spdraddress\",     (string) The spdr address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are spdr addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) spdr address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"spdraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"spdraddress\"  (string, required) The spdr address to use for the signature.\n"
