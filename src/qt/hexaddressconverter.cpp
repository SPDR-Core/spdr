...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasSpiderAddress = IsValidDestination(addr);
        if (hasSpiderAddress) {
            ui->label->setText("Address (Spider)");
        ...
        isAddressValid = hasHexAddress || hasSpiderAddress;
    }
