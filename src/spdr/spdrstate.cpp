...
SpiderState::SpiderState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = SpiderState::openDB(_path + "/spdrDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

SpiderState::SpiderState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute SpiderState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, SpiderTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createSpiderAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            spdr::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> SpiderState::vins() const // temp
{
...
void SpiderState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* SpiderState::vin(dev::Address const& _a) const
{
    return const_cast<SpiderState*>(this)->vin(_a);
}

Vin* SpiderState::vin(dev::Address const& _addr)
{
...
// void SpiderState::commit(CommitBehaviour _commitBehaviour)
// {
...
void SpiderState::kill(dev::Address _addr)
{
...
void SpiderState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address SpiderState::createSpiderAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void SpiderState::deleteAccounts(std::set<dev::Address>& addrs){
...
void SpiderState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void SpiderState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
