...
bool SpiderDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule SpiderDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t SpiderDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t SpiderDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t SpiderDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t SpiderDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool SpiderDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void SpiderDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void SpiderDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void SpiderDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void SpiderDGP::createParamsInstance(){
...
dev::Address SpiderDGP::getAddressForBlock(unsigned int blockHeight){
...
void SpiderDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SpiderDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SpiderDGP::parseStorageOneUint64(uint64_t& value){
...
void SpiderDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule SpiderDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
