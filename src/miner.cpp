...
bool ProcessBlockFound2(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SPIDERMiner : generated block is stale");
    }
 ...
 //       wallet.mapRequestCount[pblock->GetHash(usePhi2)] = 0;
    ...
    const CChainParams& chainparams = Params();
    if (!ProcessNewBlock(state, chainparams,NULL, pblock))
        return error("SPIDERMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SPIDERMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("SPIDER-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SPIDERMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
